...
public class LevelConfigYmlReader {
    ...
    private BlockLevelConfig readBlockSection(ConfigurationSection section, Material blockMatch, BlockLevelConfigBuilder defaultBuilder) {
        ...
        if (!additionBlocks.isEmpty()) {
            builder.additionalBlocks(additionBlocks.stream().map(s -> getBlockMatch(s)).collect(Collectors.toList()).toArray(new Material[0]));
        }
        return builder.build();
    }

    private Material getBlockMatch(String blockKey) {
    ...
    private void addDefaults(List<BlockLevelConfig> blocks, BlockLevelConfigBuilder defaultBuilder) {
        ...
        blocks.add(nullScore.base(Material.CAVE_AIR).build());
    }
